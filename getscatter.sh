...
for station in "${stations[@]}"; do
  ...
  echo $2
  echo $3
  if [ ! -d /usr/people/wagenaa/scatterEPS/$3/$station ]; then
    mkdir -p /usr/people/wagenaa/scatterEPS/$3/$station 
  fi
  ...
  if [ ! -d /usr/people/wagenaa/scatterEPS/$3/$station/$1-$2 ]; then
    mkdir -p /usr/people/wagenaa/scatterEPS/$3/$station/$1-$2
  fi
  #if [ ! -d /usr/people/wagenaa/CRPS_members/13-16/$station/$1-$2 ]; then
    #mkdir -p /usr/people/wagenaa/CRPS_members/13-16/$station/$1-$2
  #fi
  echo "select dtg, tastr, hmean, sigma, hobs from eps"$station"_"$3" where HOUR(timediff(tastr, dtg))>$1 AND HOUR(timediff(tastr, dtg))<$2 AND hobs IS NOT NULL" |mysql --defaults-file=/usr/people/wagenaa/.mysql/ontwrd.cnf> /usr/people/wagenaa/scatterEPS/$3/$station/$1-$2/EPS_"$station"_$1-$2_.txt 
done
)\
}
